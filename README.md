...
- `assets/`、`README.md` 与 `xingxing-ledger-mobile/`（设计工程工作区）不会随部署上传（`.vercelignore`），`index.html` 自包含全部资源。

## 功能一览

- **17 个模块**：概览、明细、代办清单、统计、报表、预算、账户、**理财**（位于账户与借贷之间）、借贷、目标、模板、周期、账本、日历、减脂、经期助手、设置
- **导航**：手机端为汉堡按钮 + 左侧滑出抽屉（280px、可点击遮罩关闭，点击模块即切即关）；平板 180px、桌面 220px 常驻侧边栏（毛玻璃），永不收起
- **模块 Banner**：左侧圆形头像（从星星人九宫格运行时裁切 9 张 512×512，白底 contain，按模块循环分配、相邻不重复）；中间标题+副标题；8 个 emoji 以 4×2 网格随机抖动散布（旋转 ±11°、17–22px、透明度 0.3–0.5），每次切换模块重新随机；右侧「📚 当前账本」切换按钮
- **视觉**：温馨可爱风设计系统（奶油暖白底、暖金主色、单色品牌 token，`--xingxing-*` 变量），卡片白底圆角、触控目标 ≥44px、安全区适配（iPhone 刘海/底部横条）
...
- 设计系统：`--xingxing-*` 品牌/中性/语义 token + 组件 CSS（不含 Tailwind 等运行时框架，无任何外部 CDN 依赖，断网可用）。
- 响应式断点：<768px 抽屉导航（280px）/ 768–1023px 180px 常驻栏 / ≥1024px 220px 常驻栏；手机内容区单栏（适配 iPhone 安全区）。
